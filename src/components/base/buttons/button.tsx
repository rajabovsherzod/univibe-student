...
export const styles = sortCx({
  common: {
    root: [
      ...
      // Disabled styles
      "disabled:cursor-not-allowed disabled:not-data-loading:text-fg-disabled",
      // Icon styles
      "disabled:not-data-loading:*:data-icon:text-fg-disabled_subtle",
...
export const Button = ({
  ...
  return (
    ...
    >
      {/* Leading icon */}
      {isValidElement(IconLeading) && <span className={cx(loading && "opacity-0")}>{IconLeading}</span>}
      {isReactComponent(IconLeading) && <IconLeading data-icon="leading" className={cx(styles.common.icon, loading && "opacity-0")} />}
      ...
      {loading && (
        <Spinner
          ...
          className={cx(
            styles.common.icon,
            "text-current", // explicitly forces tailwind to inherit surrounding white font-color
      ...
      {children && (
        <span data-text className={cx("transition-inherit-all", !noTextPadding && "px-0.5", loading && "opacity-0")}>
          {children}
        </span>
      )}

      {/* Trailing icon */}
      {isValidElement(IconTrailing) && <span className={cx(loading && "opacity-0")}>{IconTrailing}</span>}
      {isReactComponent(IconTrailing) && <IconTrailing data-icon="trailing" className={cx(styles.common.icon, loading && "opacity-0")} />}
    </Component>
  );
};
