...
export default function ForgotPasswordPage() {
  ...
  const onOtpSubmit = async (data: ForgotPasswordOtpType) => {
    const email = emailForm.getValues("email");
    try {
      const res = await verifyOtp.mutateAsync({ email, code: data.code });
  ...
  const onPwSubmit = async (data: ForgotPasswordSetType) => {
    ...
    try {
      await setPassword.mutateAsync({ password: data.password, token: accessToken });
      setStep(4);
    } catch (e: any) {
      toast.error(t("common.error"), { description: extractApiError(e) });
    }
      ...
      <div className="relative z-10 w-full max-w-[440px]">
        ...
        <div className="rounded-2xl bg-bg-secondary border border-border-secondary shadow-sm p-7">
          <AnimatePresence mode="wait">
                ...
                <form onSubmit={otpForm.handleSubmit(onOtpSubmit)} className="flex flex-col items-center gap-5">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-tertiary">
                      <strong className="text-secondary">{emailForm.getValues("email")}</strong> {t("auth.otpSent")}
                    </p>
                  </div>
            ...
            {/* ── STEP 4: Success ── */}
            {step === 4 && (
              <motion.div key="step4" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.25 }}>
